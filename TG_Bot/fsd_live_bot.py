...
# ==============================================================================
TELEGRAM_TOKEN = "8646820523:AAGjIwOjtMbf4N1wTTObDMWx-pmU13C7GOk"
ADMIN_CHAT_ID = "473643433"
...
class FSD_Math:
    ...
    @staticmethod
    def calc_entropy(p_cap: float, p_vol: float) -> float:
        def get_class(p):
            return "F" if p >= 0.65 else ("SD" if p >= 0.35 else "A")

        c_cap = get_class(p_cap)
        c_vol = get_class(p_vol)

        eff_cap = p_cap
        eff_vol = p_vol

        if c_cap == "A" and c_vol in ["A", "SD"]:
            eff_cap = 1.0 - p_cap

        if c_vol == "A" and c_cap in ["A", "SD"]:
            eff_vol = 1.0 - p_vol

        return round(eff_cap * eff_vol, 4)
...
def process_market():
    ...
    for _, r in df.iterrows():
        ...
        entropy = FSD_Math.calc_entropy(r["P_Mass"], r["P_Kin"])
        ...
        if is_bull_market and m == "F" and k == "F" and v2 == -1 and v1 == 0 and pb <= 0.15:
            ...
            long_capitulation.append(f"🟢 <b>LONG (Капитуляция): #{t}</b>\n💵 Вход: <b>${p}</b>\n🛡 SL: ${sl} | 🎯 TP: ${tp}\n📉 %B: {pb:.2f} | 🌀 Энтропия: {entropy:.2f} | <i>Умные деньги выкупают панику</i>\n")

        # 💎 LONG: Симбиоз (Бычий рынок, актив A/SD, Кинетика F, Вектор 0, зарождение тренда H > 0.55)
        elif is_bull_market and m in ["A", "SD"] and k == "F" and v1 == 0 and hurst > 0.55 and entropy <= 0.45:
            sl, tp = round(p - (1.5 * atr), 2), round(p + (3.0 * atr), 2)
            long_symbiosis.append(f"💎 <b>LONG (Симбиоз): #{t}</b>\n💵 Вход: <b>${p}</b>\n🛡 SL: ${sl} | 🎯 TP: ${tp}\n📈 Hurst: {hurst:.2f} | 🌀 Энтропия: {entropy:.2f} | <i>Скрытое накопление капитала</i>\n")
        ...
        # 🔴 SHORT: Эйфория Мусора (Медвежий рынок, актив A, Кинетика F, Вектор 0 на хаях %B)
            ...
            pe_str = f"P/E: {round(trailing_pe, 2)}" if trailing_pe is not None else "P/E: N/A"

            sl, tp = round(p + (1.5 * atr), 2), round(p - (3.0 * atr), 2)
            short_signals.append(f"🔴 <b>SHORT (Эйфория): #{t}</b>\n💵 Вход: <b>${p}</b>\n🛡 SL: ${sl} | 🎯 TP: ${tp}\n📈 %B: {pb:.2f} | 🌀 Энтропия: {entropy:.2f} | 💸 {pe_str} | <i>Институционалы разгружаются об толпу</i>\n")
